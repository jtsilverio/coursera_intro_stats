...
_In this case the shape of the distribution is better seen using 150 bins. However this number changes depending on the data we have._
...
ggplot(sfo_feb_flights) +
  ...
  geom_vline(aes(xintercept = median(arr_delay), col ="red"))
```
...
sfo_feb_flights %>%
  group_by(carrier) %>% 
  summarise(iqr_arr_delay = IQR(arr_delay))
```
...
ggplot(nycflights) +
  geom_histogram(aes(dep_delay), bins = 50)
```
...
nycflights %>% 
  group_by(dep_type) %>% 
  summarise(arr_ontime_perc = sum(arr_type == "on time")/n())
```
